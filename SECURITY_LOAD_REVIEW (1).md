...
Items are ordered by priority (severity x likelihood). "Fixed this
...
### [DONE] Auth on every route (done over prior turns)
...
### [DONE] Traceback leak fixed (`app/main.py`)
...
### [DONE] Email recipient guard (`app/services/email_send.py`)
...
### [DONE] Input-size caps on AI-bound fields (chat, assistance, market_research, engagement)
...
### [DONE] Scan loop: hoisted the per-user listings query (`app/services/scheduler.py`)
