...
@router.get("/candidate-evidence/{candidate_user_id}")
def get_candidate_evidence(candidate_user_id: str, db: Session = Depends(get_db)):
    """Real evidence of grit and curiosity for a business reviewing a
    specific candidate - never a fabricated score. Grounded in the
    candidate's real, timestamped application history (a genuine
    persistence fact) and their own words from career_discovery's
    evidence-based survey (a real, quoted curiosity signal), if
    available. See app/services/candidate_evidence.py for the full
    reasoning on why this deliberately never outputs a "Grit: 87/100"
    style score.
    """
    from app.models.db_models import Outcome, CareerDiscoveryResult
    from app.services.candidate_evidence import compute_persistence_signal, generate_grit_curiosity_evidence
 
    outcome_rows = db.query(Outcome).filter(Outcome.user_id == candidate_user_id).all()
    outcomes = [{"status": o.status, "updated_at": o.updated_at} for o in outcome_rows]
    persistence = compute_persistence_signal(outcomes)
 
    discovery_result = (
        db.query(CareerDiscoveryResult)
        .filter(CareerDiscoveryResult.user_id == candidate_user_id)
        .first()
    )
    free_text = (discovery_result.answers or {}).get("free_text") if discovery_result else None
 
    try:
        evidence = generate_grit_curiosity_evidence(client, persistence, free_text)
    except Exception as e:
        raise HTTPException(status_code=502, detail=f"Could not generate candidate evidence just now: {e}")
 
    return {"persistence_stats": persistence, **evidence}
